...
with st.sidebar:
    ...
    st.write("---")
    st.subheader("📌 선택한 센터 (최대 5개)")

    if not st.session_state.clicked_centers:
        st.caption("지도에서 핀을 누르면 여기에 표시됩니다.")
    else:
        for i, item in enumerate(st.session_state.clicked_centers, 1):
...
if should_search:
    ...
    if clicked and data_list:
        ...
        if center_row:
            ...
            # 중복이면 기존 항목 제거 후 맨 앞에 재삽입 (최근 클릭 우선)
            st.session_state.clicked_centers = [
                x for x in st.session_state.clicked_centers if x.get("id") != cid
            ]
            st.session_state.clicked_centers.insert(0, {"id": cid, "name": cname})

            # 최대 5개 유지
            st.session_state.clicked_centers = st.session_state.clicked_centers[:5]
